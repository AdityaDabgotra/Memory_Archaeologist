  ...
  const send = async () => {
    ...
    try {
      ...
      const hit = concepts.find((c) =>
        res.answer.toLowerCase().includes(c.concept)
      );
  ...
  const uploadFile = async (e) => {
    ...
    try {
      ...
      setMessages((p) => [
        ...
        {
          role: "assistant",
          content: ` **${data.filename}** ingested successfully — ${data.chunks} chunks added to memory.`,
        },
      ]);
      // Refresh stats
      getConcepts().then((d) => setConcepts(d.concepts));
      ...
      setMessages((p) => [
        ...
        {
          role: "assistant",
          content: " Upload failed. Check the server logs.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflow: "hidden" }}>
        ...
        {tab === "graph" && (
          <div
            style={{ height: "100%", padding: "24px", background: "var(--bg)" }}
          >
            <div
              style={{
                height: "100%",
            ...
            >
              {concepts.length > 0 ? (
                <KnowledgeGraph
                  concepts={concepts}
                  highlightedConcept={highlighted}
                />
              ) : (
                <div
                  style={{
